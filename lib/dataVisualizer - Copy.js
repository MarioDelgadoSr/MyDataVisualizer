		...
		var GDRIVE_DEMO = true; //CORS workaround for demonstration: https://en.wikipedia.org/wiki/Cross-origin_resource_sharing	
	
		var appVersion = 0.9;
		
		var orbitControls;
		var container, camera, scene, renderer, loader;
		var gltfData, visualData;
		...
		function fnBuildDataVisualizerLayout(container, folderIndex, languageIndex){
			...
			panels: [
				{ type: 'left', resizable: true, size: "50%", style: pstyle, title:'Visualization Scale',
		...
		function fnGetObjectListAndLoad(folderIndex, languageIndex) {
			...
			if (GDRIVE_DEMO){	
				...
				//var folderList = {columns: ["Body","Forklift","Plane","Ship","Truck"]};
				//var folderList = {columns: ["Forklift","Body","Plane","Ship","Truck"]};
				var folderList = {columns: ["Forklift","Body","Plane","Ship","Truck"]};
				/*
				folderList.columns.forEach(function(file){
						
						window[ file + "Data"].forEach(function(record){
							d3.autoType(record); //https://github.com/d3/d3-dsv/blob/master/README.md#autoType	
						});
					
						window[ file + "Data"].columns = Object.keys(window[ file + "Data"][0]);
		...
		function fnLoadDemoOrFileData(aFiles,indexFile){
				...
				//Start CORS workaround for demonstration ******************************* https://en.wikipedia.org/wiki/Cross-origin_resource_sharing
				
				if (GDRIVE_DEMO){
			
					visualData = window[aFiles[indexFile] + "Data"]	//https://github.com/d3/d3-dsv/blob/master/README.md#autoType						
	
	
					//GLB packer: http://glb-packer.glitch.me/ 
					//Encoder: https://www.base64encode.org/
					//Encode/decode with W2UI: http://w2ui.com/web/demos/#!utils/utils-2
					var b64Data = window[aFiles[indexFile] + "GLTF"];
					
					var contentType = 'model/gltf-binary';   //https://github.com/KhronosGroup/glTF/issues/943				
					
					var blob = b64toBlob(b64Data, contentType); //https://stackoverflow.com/questions/16245767/creating-a-blob-from-a-base64-string-in-javascript
																
																//https://developer.mozilla.org/en-US/docs/Web/API/URL
					var blobUrl = URL.createObjectURL(blob);   //https://discourse.threejs.org/t/solved-how-can-we-load-an-obj-file-from-a-data-url/3701/5
					
						//GLTF Loader
						
						gltfLoader = new THREE.GLTFLoader();
				
						gltfLoader.load( blobUrl, function( gltfDataFromFile ) {  //https://discourse.threejs.org/t/solved-how-can-we-load-an-obj-file-from-a-data-url/3701/5
						
							gltfData = gltfDataFromFile; //gltfData is global
												
							fnLoadsComplete();  //Data already refrenceable in Javascript variable	
						...
						undefined, function ( error ) {   //Error Handling
							...
							w2alert(translations[localLanguage].errorLoadingFile + blobUrl + " " + error );
				...
				//End CORS workaround for demonstration *******************************
		...
		function fnLoadData(aFiles,indexFile) {
				...
				bInitiating =  true;   //Re-set initiating Flag, used id data is being reloaded
		
		
				if (GDRIVE_DEMO) {
				
					return;  //Data doesn't change for Demo
				}
				else {
		...
		function fnLoadsComplete() {
						...
						if (!visualizer) {
							...
							visualizer.setGltfData(gltfData);
						...
						if (visualData) {
							...
							visualizer.addSelectionLink({url:"https://www.google.com/search?" , urlText:"Google Search", "q":keyDataName});									
																					  ...
																					  //Assuming setGltfData and setVisualData, Join data to visual			
							...
							if (!visualizer.scene.obj) {	//In the event fnLoadsComplete is being called recursively	
								...
								visualizer.setCallBackReload(fnLoadData);    			//A callback for refreshing the data                      													
...
function dataVisualizer (overrideProperties) {
	...
	var properties = {//Default visual layout props; attributes can be changed with attributes in ojb3dProperties
		...
		gltfData: null,																					//The gltf file to be loaded
	...
	function setGltfData(gltfData){
		properties.gltfData = gltfData;	
	} //setGltfData
	...
	function fnBuildDataGrid(){
		...
		if (!properties.dataRefreshing) {
			...
			// $.extend(true,properties.dataGrid,properties.dataGridProps);  //Won't work, replaces toolbar.items array rathern than merging it
	...
	function fnBuildVisual() {
		...
		var visual = new THREE.Group();  															//Containing the whole image allows for possible complete transforms later
		visual.name = "visual";
																										//In Threejs space, image is laid out relative to 0,0,0		
		var	gltfData = properties.gltfData;

		//gltfData.scene.traverse( function ( node ) {

			//if ( node.isMesh || node.isLight ) node.castShadow = true;
			
		//} );
		
		//var meshes = gltfData.scene.children.filter(function(node){return node.isMesh || node.isGroup});	
		//meshes.forEach(function(mesh){visual.add(mesh.clone())});	
		visual.add(gltfData.scene.clone());	

		visual.traverse(function (node) {  //object may be a node deep in the mesh hiearchy	
			if (node.userData){
	...
	function fnGetDataGridProperties(data) {
		...
		columns.push({field:properties.keyDataName, caption: properties.keyDataName + " (Key/Scaled)", //Always in column 2
		...
		dataFields
			...
			.forEach(function(field,i){ 
							...
							if (field != properties.keyDataName){
								var column = {field:field, caption:field, sortable: true, searchable: true, hidden: false, render: fnDataGridColumnRender};
								columns.push(column);
		...
		return {
			...
			toolbar: {
				items: [
					{type: 'break'},
					{type: 'check', id: 'styleGrid', caption: "Visualize Grid Data", checked: false, tooltip: 'Visualize Data on Grid'},
					{type: 'break'},
		...
		function fnHandleSelectionOrSearch(event){
			...
			var recIds = event.searchData ?  w2ui[event.target].last.searchIds :   // http://w2ui.com/web/docs/1.5/w2grid.last
												properties.dataGrid.getSelection(true); // http://w2ui.com/web/docs/1.5/w2grid.getSelection
				
			recIds.forEach(function (recid){
								
				var selectedObject = properties.visualGroup.getObjectByProperty(properties.keyVisualName, properties.dataGrid.records[recid][properties.keyDataName]); //the join
	...
	function fnGetVisualGridProperties(visualObjectMap) {
		...
		return {
			...
			show: 		{
							...
							toolbarReload: false,
							footer: true,
							selectColumn: false, //Not a feature in this version
							footer: true,							
						},
			toolbar: {
				items: [
					...
					{type: 'color', id: 'bBoxColor', color: properties.scene.obj.background.getHexString() , caption: "Color", tooltip: "Set Background Color"}, 													
					{type: 'break'},
		...
		function fnColorVisualObject(column,fnCalcColor, visualizeData, dataKey){
			...
			if (visualizeData){
				...
				for (var dataIndex = 0; dataIndex < visualizeData.length; dataIndex++){
					...
					var color = fnCalcColor(dataIndex, column);  //Could be a string, or an object (with or without children) from original gltfData
