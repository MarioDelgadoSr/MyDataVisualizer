		...
		var appContainer, visualizerDiv, legend;	
		...
		var languages = ["English","Spanish","Swedish","Finnish","French","Danish","Lithuanian","Polish","Indonesian","Filipino","Chinese","Czech","Dutch","Estonian","German","Greek","Hebrew","Icelandic","Italian","Japanese","Korean","Latvian","Norwegian","Persian","Portuguese","Romanian","Russian","Serbian","Slovak","Slovenian"];
		var languageCodes = ["en","es","sv","fi","fr","da","lt","pl","id","fil","zh-CN","cs","nl","et","de","el","he","is","it","ja","ko","lv","nb","fa","pt","ro","ru","sr","sk","sl"];
		...
		function fnBuildDataVisualizerLayout(container, folderIndex, languageIndex){
			...
			localLanguage = languageCodes[languageIndex];
			...
			$(container)
				...
				visualizerDiv = visualizerDiv ?														//reuse it if it exists
									visualizerDiv : 
									$(document.createElement("div")).attr("id","visualizerDiv"); 	//Global JQuery reference to right panel, DOM reference is visualizerDiv[0]
		...
		*/
				...
				$().w2toolbar({	name: "toolbar",
					items:[				
						...
						{ type: "menu-radio", id: "language", 
						  items: languages.map(function (language) { return {id: language ,caption: language} }),  
						  selected: languages[languageIndex],
						  caption: function (item){
						...
						event.done( function () {
							...
							switch(aTargets[0]) {
								...
								case "language":
									...
									fnBuildDataVisualizerLayout(appContainer, folderList.indexOf(folderName), languages.indexOf(aTargets[1]) ); //rebuild everything for new language
									break;
								case "help"	:
									switch (aTargets[1]){
										case "quickKeys":
		...
		function fnLoadsComplete() {
						...
						if (!visualizer.dataRefreshing) { //Skip Display if data is being refreshed		
							...
							var container = visualizerDiv[0];		//DOM reference to the JQuery layout container hosting the visualization
