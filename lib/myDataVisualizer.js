...
$(document).ready(function(){
	...
	gltfLoader.load( blobUrl, function( gltfDataFromFile ) {  
		...
		var datVisual =  new dataVisual();	
			...
			datVisual.joinDataToVisual(ForkliftData, gltfDataFromFile, dataKey, visualKey, "COLOR_"); 			//datVisual.joinDataToVisual(gltfDataFromFile);  //testing with no data	
						...
						name: dataVisual.visualName + "_mainPanel", 																			//http://w2ui.com/web/docs/1.5/layout
								panels: 
								  [																												//Tool Bar Handle by Main Panel
									...
									{ type: 'preview',  size: '75%', resizable: true, toolbar: {} }  											//The display panel for the image
								] //panels
					}					
				); //layout
				
		layout.hideToolbar("preview");																											//Will be shown if no data	
		...
		*/
		...
		function fnFinshLayout(){
			...
			if (dataVisual.data.length == 0){  																								//No Data, just image
				...
				var toolbar = fnGetGridToolbar(dataVisual, mdv.visualizer,"visual");
					...
					mdvLayout.get("preview").toolbar.add(toolbar.items.filter(function(item,i){return i > 0}));
					mdvLayout.get("preview").toolbar.onClick = toolbar.onClick;
		...
		function fnBuildDataVisualizer(container, dataVisual){
			...
			function dataVisualizer (overrideProperties) {
				...
				var properties = {//Default visual layout props; attributes can be changed with attributes in ojb3dProperties
					...
					tooltip: {	mouse: new THREE.Vector2() },														//Tooltip object 					
				...
				function fnShowToolTip(dataVisual){ // https://github.com/stemkoski/stemkoski.github.com/blob/master/Three.js/Mouse-Tooltip.html
					...
					if ( intersects.length > 0 ){
						...
						if ( intersects[ 0 ].object != properties.tooltip.interesected ) {
						
							// store reference to closest object as current intersection object
							properties.tooltip.interesected = intersects[ 0 ].object;
							...
							if (dataRow){
								...
								var message = dataRow[dataVisual.dataKey];
				...
				function fnBuildScene() {	
					...
					properties.tooTipDiv = $(document.createElement("div"));

					properties.tooTipDiv.css({
										  position: "absolute",
										  left: 0,
										  top: 0,
										  "text-align": "center",
					...
					$(container).append(properties.tooTipDiv);
					...
					properties.fnHandleKeyMouse = function (event){
						...
						switch(event.type){
							...
							case "dblclick":
								...
								if (intersects) properties.fnSearchIntersects(intersects, true);
								break;
							case "mousemove":	  //https://stackoverflow.com/questions/39177205/threejs-tooltip http://jsfiddle.net/UberMario/60xkg97p/4/
							
									handleManipulationUpdate(event, properties.mouse, properties.tooTipDiv);		

							default:	
						} //switch
						
						function handleManipulationUpdate(event, mouse, toolTip) {
							...
							var raycaster = properties.raycaster;
							
							raycaster.setFromCamera(mouse, camera);



							var intersects = raycaster.intersectObjects(dataVisual.join.map(function(joinRow){return joinRow.visualObj}));
				...
				function fnGridColumnRender(record, row_index, column_index) {	
					...
					var color = predDefinedColor ? 
							record[this.columns[column_index].field] : 
								fnGetObjectScaleColorAndOptionallyVisualize(dataVisual, fieldValueColumn, dataRecordIndex, key);   
		...
		function fnSetVisualColors(dataVisual){  // fnSetVisualColors() with no parms resets visualizations
			...
			if (dataVisual){																					
				...
				var colorColumn = fnIsPresetColor(column) ?    column.split(dataVisual.colorPrefix)[1] : column;  //Reference to origial column to get it's unique categories, not the column with the colors		
