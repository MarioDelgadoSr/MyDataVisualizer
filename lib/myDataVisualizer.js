										...
										switch(aTargets[0]) {
											...
											case "Help"	:
												...
												switch (aTargets[1]){
													...
													case "w2ui":
															var links = { 
																		"documentation": "https://github.com/MarioDelgadoSr/MyDataVisualizerDoc/blob/master/README.md#my-data-visualizer-documentation",		
...
function fnShowMyDataVisualizerDemo(){
	...
	gltfLoader.load( gltfURL, function( gltfDataFromFile ) {  
		...
		if (!appVisuals[visualIndex].blob) {    //demo visual, could have links at the dataVisual.join level when dataVisual.urlPrefix column is in data
		
			var linkURL = "https://www.bing.com/search";																			
			datVisual.selectionLinks.push( {url: linkURL + "?" , urlText:"Bing Search: " + dataKey  , "q":dataKey}); // Free version will allow 1 link of simple url				
			
			/*	For testing				
			datVisual.join.forEach( (join) => {
										join.selectionLink = {url: "http://www.bing.com/search?q=", urlText:"http://www.bing.com/search?q=" };		
									 });													
			*/																						
															
		} //if
		
		/* Python equivalent, where dataKey == "name":
		
			bpy.data.scenes['Scene']['links'] = [dict() for x in range(1)]
			bpy.data.scenes['Scene']['links'][0]["url"] = "http://www.bing.com?"
			bpy.data.scenes['Scene']['links'][0]["urlText"] = "Bing Search: name"
			bpy.data.scenes['Scene']['links'][0]["q"] = "name"
...
function fnHandleFileUploads (event) {				
	...
	if (files.length == 2){
		...
		var gltfFile = fnParseGLFile(files[0]) ? files[0] : fnParseGLFile(files[1]) ? files[1] : null;

		if (!csvFile && !gltfFile){
			
			w2alert("Only .csv and .gltf/.glb files can be processed.")
		...
		reader.onloadend = function() {			//https://w3c.github.io/FileAPI/#filereader-result
			...
			var visualName = fnParseGLFile(gltfFile);
			...
			appVisuals.push({	url: null,
								...
								data: d3.csvParse(this.result, fnParse),   		//https://github.com/d3/d3-dsv#csvParse 
								dataName: dataName,
		...
		var visualName = fnParseGLFile(files[0]);		
		
		if (!visualName) {
		
			w2alert("A .gltf or .glb file was not uploaded.")
	...
	function fnParseGLFile(file){
		
		var gltfIndex = file.name.indexOf(".gltf");
		var glbIndex =  file.name.indexOf(".glb");
		
		if (gltfIndex == -1 && glbIndex == -1){ 
			...
			return  file.name.substring(0, gltfIndex != -1 ? gltfIndex : glbIndex );
		...
		*/
		...
		function fnFinshLayout(){
			...
			if (dataVisual.data.length == 0){  																								//No Data, just image
				...
				w2ui.parentLayout.get("main").toolbar.click("toggleVisualGrid");															//Initially Hide Visual Grid	
		...
		function fnSearchIntersects(intersects, bSearch){
			...
			if (intersects == "reset"){
				...
				var dataRow =  fnFindDataRow(intersects);
			...
			function fnFindDataRow(intersects){
				...
				//Use Join to determine keyValue; if an intersect contains one of the visualization objects
				var dataRow = null;
		
				for (var i = 0; i < intersects.length; i++){
					
					var bFoundMatch = dataVisual.join.some(function(joinRow){
				...
				function isVisualObj(visualObj, intersectObj){
					...
					intersectObj.traverseAncestors(function(node){
							...
							// Keep checking ancestors/parents to determine if instersect is a child of a visualObj
							bVisualObj = bVisualObj ? bVisualObj : visualObj === node;  			
		...
		function fnBuildDataVisualizer(container, dataVisual){
			...
			function dataVisualizer (overrideProperties) {
				...
				var properties = {//Default visual layout props; attributes can be changed with attributes in ojb3dProperties
					...
					visualizeProp: "visualize",																	//The property in the GLTF file that designates meshes to visualize
				...
				function fnGetResizeCallback(){
					...
					//Referenced through debug code: appLayout.myDataVisualizer.visualizer.resize();
				...
				function fnBuildScene() {	
					...
					properties.controls.obj.maxDistance = properties.gridHelper.gridSize * 1.5;						//Don't allow orbital control to go out beyond 150% of gridSize		
					...
					*/
					...
					function fnSaveChildrenOriginalColor(node){   //parent and children (if any) have userData.originalColor assigned to them as well
							node.traverse(function (node){
								if (node.material) node.userData.originalColor = new THREE.Color().copy(node.material.color);	//copy the original color associated with the mesh		
							});
		...
		function fnBuildGrid(dataVisual, visualizer, gridType){
			...
			function fnGetGridProperties(dataVisual, visualizer, gridType) {
				...
				//dataVisual.getJoinByKey(dataRow[dataVisual.dataKey]).selectionLink 
				...
				case "dataGrid":
					...
					return {
								items: [
										...
										// {type: 'break'},
										// {type: 'button', id: 'downloadGLTFButton', text: "Download glTF", tooltip: 'Download .gltf file of visual with embedded data'} 
												...
												switch(event.target){
													...
													case "downloadDataButton":	
														...
														if (!( document.documentMode || /Edge/.test(navigator.userAgent ) ) ) {	
															...
															if (anchor.length == 0){
																...
																var anchor = $(document.createElement("a"))
																				.attr("download",dataVisual.dataName +  (dataVisual.dataName.indexOf(".csv") == -1 ? ".csv" : "") );  
														...
														function fnFilterDownLoadData(data){
															...
															var columns = Object.keys(data[0]).filter( (column) => !dataVisual.dataTypes[column].isVisualData );	
													...
													case "downloadGLTFButton":	//Possible future feature 
														...
														if (!( document.documentMode || /Edge/.test(navigator.userAgent ) ) ) {	
															...
															if (anchor.length == 0){
																...
																var anchor = $(document.createElement("a"))
																				.attr("download",dataVisual.visualName + ".gltf");  ;  
...
//Reference:  http://www.crockford.com/javascript/private.html		
	...
	this.joinDataToVisual = function (data, visual, dataKey, visualKey, colorPrefix, datePrefix, dateTimePrefix, timePrefix, urlPrefix){     //Paying customers can override default color and date prefix
		...
		/* 	
			...
			visualKey can be either an attribute of the mesh or mesh.userData.  It will try finding it via mesh[visualKey] before searching in mesh.userData[visualKey]
	...
	this.setColorVisualObj = function(visualObj, color){
		...
		visualObj.traverse(function(node) {  																		//Color the object and any child meshes with color-able materials	
			if (node.type == "Mesh" && node.material) {
					...
					Array.isArray(node.material) ? node.material.forEach((m) => m.color.set(color)) :
