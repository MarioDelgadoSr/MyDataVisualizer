...
$(document).ready(function(){
	...
	gltfLoader.load( blobUrl, function( gltfDataFromFile ) {  
		...
		var datVisual =  new dataVisual();	
			...
			datVisual.joinDataToVisual(ForkliftData, gltfDataFromFile, dataKey, visualKey); 					//datVisual.joinDataToVisual([], gltfVisual, dataKey, visualKey);  //testing with no data	
		...
		*/
		...
		function fnFinshLayout(){
			...
			dataVisual.visualGrid = mdv.visualGrid;
																																			//Includes data from visual grid
			...
			if (dataVisual.data.length == 0){  																								//No Data, just image
				...
				dataVisual.dataGrid.activeGridColumn = dataVisual.dataKey;																			//First column to be visualized	
				...
				return {
						...
						onClick: function (target, data){
									...
									data.onComplete = function (event){
										...
										if (event.target == "styleGrid"){   //Button to Toggle Visualization Styling of Data Grid Selected
											...
											var column = this.owner.getColumn(dataVisual.dataGrid.activeGridColumn, true);
					...
					onColumnClick: function(event){ 
						...
						event.done(function(){	
							...
							dataVisual.dataGrid.activeGridColumn = event.field; 
							
							fnVisualizeData(event.field, dataVisual, dataVisual.dataKey);            
							fnVisualScaling(event.field, dataVisual, dataVisual.dataKey);								  
							
							fnGridSelectColumn(this, dataVisual.dataGrid.activeGridColumn);
		...
		function fnBuildVisualGrid(dataVisual,visualizer){
			...
			function fnGetVisualGridProperties(dataVisual, visualizer) {
				...
				var columns = [{field:"recid", caption:"Object id", sortable:true, searchable:true}];
			...
			scaleSlider.addSlider({
				...
				changed: function (v) {
					...
					timeOutVar = setTimeout(function(){
												...
												dataVisual.dataGrid.columnClick(dataVisual.dataGrid.activeGridColumn);  //This rebuilds the grid, which inturn updates the scale via fnVisualScaling						
											}
											,timeOutInterval
											); //setTimeout
