	...
	var appVisuals = [];																												//Array of objects for demo and uploaded visuals

$(document).ready(function(){

	visualList.forEach(function(visual){
	...
	$(document.body).css({"margin":"0px", "position": "absolute", "width": "100%", "height": "100%"})						// https://github.com/vitmalina/w2ui/issues/105 
	...
	parentLayout = 
		$().w2layout(																											
				{	
					...
					panels: 
					  [																											
						{ type: 'top', size: "100%", resizable: false, style: pstyle, 
							toolbar: {	
								items:[	
									{type: "button", id:"upload", tooltip: "Upload Visual", caption:"Upload"},	
									{type: 'break'},									
									{ type: "menu-radio", id: "visual", tooltip: "Select Visual",
									  items: visualList.map (function (visual, index) { return { id: index, caption: visual} }),
									  //Find index programmatically with: w2ui.parentLayout.get("top").toolbar.items.find(function(item){return item.id == "visual"}).selected
									  selected: 0,  
									  caption: function (item){
												return "Visual: " + item.items[item.selected].caption;
											}
									},
									{type: 'break'},
									{ type: "menu-radio", id: "language", tooltip: "Select Language",
									  items: languages.map(function (language,index) { return {id: index ,caption: language.text} }),  
									  //Find index programmatically with: w2ui.parentLayout.get("top").toolbar.items.find(function(item){return item.id == "visual"}).selected
									  selected: languageIndex,
									  caption: function (item){
												return "Language: " + item.items[item.selected].caption;
											}	
									},
									{type: 'break'},
									{ type: "menu", id: "help", caption: translate("mdvHelp"), tooltip: "Help",
									  items: [
											...
											{ id: "about",     caption: translate("mdvAbout")}
									   ]	
									}
								],
								onClick: function(event){
									event.done( function () {
										...
										if (aTargets.length == 1 && aTargets[0] != "upload") return; //Menu, not menu item, clicked on option
										switch(aTargets[0]) {
											case "visual":   //Find programmatically with: w2ui.toolbar.items.find(function(item) {return item.id == "visual"}).selected

												var visualIndex = this.items.find(function(item){return item.id == "visual"}).selected;
											...
											case "help"	:
												...
												switch (aTargets[1]){
													case "quickKeys":
														...
														var helpText = translate("mdvHelpText2") +  translate("mdvHelpText3")  + translate("mdvHelpText1") + translate("mdvHelpText4");			  
														fnPopUp(document.body, translate("mdvHelp"),helpText,3,2.5);									
														break;
														
													case "about":
													
														fnPopUp(document.body, translate("mdvAppTitle") + " (" + appVersion +")",translate("mdvAboutText"),3,5);
														break;
										...
										function fnPopUp(appContainer, title,html,wf,hf){ 
											...
											w2popup.open({	title: title,
															body: html,
															buttons   : '<button onclick="w2popup.close();">' + translate("mdvPopUpClose") + '</button>',	
															showMax: false,
															showClose: true,
															width: width/wf,
															height: height/hf,
...
function fnShowMyDataVisualizerDemo(){
	...
	var visualIndex = w2ui.parentLayout.get("top").toolbar.items.find(function(item){return item.id == "visual"}).selected;
	...
	gltfLoader.load( gltfURL, function( gltfDataFromFile ) {  
		...
		var datVisual =  new dataVisual();	
			...
			if (visualData) {	
				datVisual.joinDataToVisual(visualData, gltfDataFromFile, dataKey, visualKey, colorPrefix); 			//datVisual.joinDataToVisual(gltfDataFromFile);  //testing with no data	
			}
			else {
			...
			var container = typeof(parentLayout) != 'undefined' ? parentLayout : 								//Continer can be a div or layout.  If layout, app placed in 'top' panel
...
function fnHandleFileUploads (event) {				
	...
	if (files.length == 2){
		...
		reader.onloadend = function() {			//https://w3c.github.io/FileAPI/#filereader-result
			...
			parentLayout.get("top").toolbar.get("visual").items.push({id: appVisuals.length - 1, caption: visualName, checked: true});
			parentLayout.get("top").toolbar.get("visual").selected = appVisuals.length - 1;
...
function applicationLayout(){
	...
	app.display = function(container, dataVisual){
		...
		if (w2ui[container.name]){
			...
			var mdvLayout = $().w2layout(mdvLayoutProperties); 
				//container.set("top",{content: mdvLayout });	
				container.content("top", mdvLayout );			
	...
	app.destroy =  function (){
		...
		[ 	app.dataVisual.visualName + "_dataGrid",
			app.dataVisual.visualName + "_visualGrid",
			app.dataVisual.visualName  + "_scaleControlLayout",
			app.dataVisual.visualName  +  "_analyzerLayout",
			app.dataVisual.visualName + "_mainPanel_preview_toolbar",			
			app.dataVisual.visualName + "_mainPanel"
		].forEach(function(w2Resource){ if (w2ui[w2Resource]) w2ui[w2Resource].destroy();  });	
		...
		*/
						...
						function fnHandleMouseMove(toolTip) {
							...
							if (intersects) {
								...
								if (dataRow){
									...
									var caption = dataVisual.activeGridColumn.grid.columns.find(function(col){return col.field == column}).caption
									
									var columnText = (fnIsPresetColor(column) || dataVisual.data[0][dataVisual.colorPrefix + column] != undefined ) ? columnText0 + caption : caption;
									var formattedValue = dataVisual.dataTypes[column].type == "float" ? d3.format(",")(dataRow[dataVisual.activeGridColumn.column] ) :
		...
		function fnGetDataTypes(visualizeData) {	
			...
			aKeys.forEach(function(key) { 
				...
				dataTypes[key].aUniqueCategories = 
							Array.from(new Set(visualizeData.map(function (row) {return dataTypes[key].type == "float" ? parseFloat(row[key]): row[key]}) ))
		...
		function fnBuildGrid(dataVisual, visualizer, gridType){
			...
			function fnGetGridProperties(dataVisual, visualizer, gridType) {
				...
				var dataFields = Object.keys(dataVisual.data[0]).filter( function(field){
																			parseField = field.split(dataVisual.colorPrefix);
																			field =  parseField[parseField.length - 1];
				...
				var key = (gridType == "data" ? "" : " ")  + dataVisual[gridType + "Key"] ;
				
				columns.push({field:"recid", caption:"Join Row", sortable:true, searchable:true, hidden: false, render: fnGridColumnRender });   //Always located in column 1 associted with key used for join in Column 2
				
				columns.push({field:key, caption: key.trim() + " (Key)", //Always in column 2
				...
				dataFields
					...
					.forEach(function(field,i){ 
									...
									if (field != key){
										var column = {field:field, caption: field.trim()  + " (Scaled)", sortable: true, searchable: true, hidden: false, render: fnGridColumnRender};
										columns.push(column);
									...
									if (dataTypes[dataVisual.colorPrefix + field]){ //If the current field in loop also has predefined color; position here for column grouping
										...
										var colorColumn = {field: dataVisual.colorPrefix + field, hidden: false, 
															caption: "Color (Preset)", sortable: true, searchable: false, render: fnGridColumnRender};
					...
					for (var i = 1; i < columns.length - 1; i++){  //skip over recid and last columm ( i < columns.length - 1)
						...
						if ( !fnIsPresetColor(columns[i].field) && !columns[i].hidden ){
							...
							if (dataTypes[dataVisual.colorPrefix + columns[i].field]){  //If the current field as a preset color associate with it
								...
								columnGroups.push({caption: columns[i].field.trim() , span: 2, master: false});  //Span over current field and next field
								
							}	
							else {								
									
								columnGroups.push({caption: columns[i].field.trim(), span: 1, master: true}); //Otherwise, just a single splan
				...
				function fnGridColumnRender(record, row_index, column_index) {	
					...
					var fieldText = fieldColorColumn == "recid" ? record.recid  : record[fieldColorColumn];  //Add 1 to zero-based recid for display
				...
				case "data":
					...
					return {
							items: [
									...
									{type: 'break'},
									// {type: 'button', id: 'downloadButton', caption: translate("mdvDownLoadData"), tooltip: 'Download .csv file of current data grid'} 
		...
		function fnSetVisualColors(dataVisual){  // fnSetVisualColors() with no parms resets visualizations
			...
			if (dataVisual){																					
				...
				/*	
				var columnText = fnIsPresetColor(column) ? column.split(dataVisual.colorPrefix)[1].trim() + 
																" " + dataVisual.activeGridColumn.grid.columns.find(function(col){return col.field == column}).caption
