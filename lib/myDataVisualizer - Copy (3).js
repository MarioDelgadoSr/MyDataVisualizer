...
$(document).ready(function(){
	...
	gltfLoader.load( blobUrl, function( gltfDataFromFile ) {  
		...
		var datVisual =  new dataVisual();	
			...
			//datVisual.joinDataToVisual(ForkliftData, gltfDataFromFile, dataKey, visualKey); 					//
			datVisual.joinDataToVisual(gltfDataFromFile);  //testing with no data	
		...
		*/
		...
		function fnFinshLayout(){
			...
			fnResizeMdvLayout = function(event) {  																							//Resize visualizer on layout's resize
									...
									event.onComplete =	mdv.visualizer.resize(); 															//Exposed by dataVisualizer for resizing 
			...
			*/
			...
			if (dataVisual.data.length == 0){  																								//No Data, just image
				...
				//const toolBarHeight = 70;																									//Via Analysis of elements built
				//var totalHeight = mdvLayout.get("main").height + mdvLayout.get("preview").height;
				//mdvLayout.get("preview").size = parseInt((totalHeight - toolBarHeight)/totalHeight * 100) + "%";
				...
				var toolbar = fnGetVisualGridToolBar(dataVisual, mdv.visualizer);
		...
		function fnBuildVisualGrid(dataVisual,visualizer){
			...
			var visualGridProps  = fnGetVisualGridProperties(dataVisual, visualizer); 													//Build the visualGrid
			...
			function fnGetVisualGridProperties(dataVisual, visualizer) {
				...
				if (dataVisual.data.length > 0 ) {	
					...
					//Difference in arrays: https://stackoverflow.com/questions/33356504/difference-and-intersection-of-two-arrays-containing-objects?answertab=active#tab-top
				...
				return {
					...
					multiSelect: true,
					multiSort: true,
					toolbar: fnGetVisualGridToolBar(dataVisual, visualizer),
					onColumnClick: function(event){ 
		...
		function fnGetVisualGridToolBar(dataVisual, visualizer) {
